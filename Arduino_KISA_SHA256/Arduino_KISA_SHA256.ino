...
void  setup() {
  ...
  Serial.println(" ");  Serial.println("20181013YouKyoung Code Start..");  Serial.println(" ");  
  loop_num = 0;
  bufferPosition = 0;
  count = 0;
}
...
void  loop() {  
      ...
      if(Serial.available()){
         ...
         IN_MESS[bufferPosition++] = c;
        
         if(c == '\n'){
             IN_MESS[bufferPosition]='\0';
             count = -1;
          }

         if( loop_num == 0 && count == -1)    // Display just one time
        {   
          Serial.print("Input Message: ");  Serial.println((char *)IN_MESS);   
          Serial.print("Input Length: ");  Serial.println( bufferPosition-1);
          SHA256_Encrpyt( IN_MESS, bufferPosition - 1, OUT_DST );  

          Serial.print("Calculated Digest Value:  ");
          for (int i=0; i<32; i++)    
          {   Serial.print(OUT_DST[i], HEX);  Serial.print(", ");  }   Serial.println(" ");
      
          Serial.print("TestVector Digest Value:  ");
          for (int i=0; i<32; i++) 
          {   Serial.print(t_value[i], HEX);  Serial.print(", ");  }   Serial.println(" ");
